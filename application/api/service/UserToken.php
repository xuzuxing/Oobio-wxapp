...
class UserToken extends Token
{
    ...
    {
        $this->code         = $code;
        $this->rawData      = json_decode($rawData);//转对象
    ...
    private function newUser($openid)
    {
        ...
        $user = User::create(
            [
                ...
                'avatarUrl' => $this->rawData->avatarUrl,
        ...
        $uid = $user->id;
        //创建积分表
        $intId = $this->createIntegral($uid);
        //创建积分操作表
        $this->createIntrgralItem($uid,$intId);
        return $uid;
    }
     ...
     * @return array
     */
    private function createIntegral($uid){
        $data = [
            'uid'   => $uid,
            'total_points'  => 20,
            'gift_points'   => 20,
     ...
     * @param $intId
     */
    private function createIntrgralItem($uid,$intId){
        //创建积分操作表
        $points = [
            'pid'           =>  $intId,
            'uid'           =>  $uid,
